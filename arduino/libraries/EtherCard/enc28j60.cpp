...
void ENC28J60::initSPI () {
    // these are defined by Arduino.h/pins_arduino.h
...
byte ENC28J60::initialize (word size, const byte* macaddr, byte csPin) {
    ...
    if (bitRead(SPCR, SPE) == 0)
      initSPI();
    selectBit = digitalPinToBitMask(csPin); // we use selectBit as pin number
    pinMode(csPin, OUTPUT);
    digitalWrite(csPin, HIGH);	// digitalWrite disables PWM too
    disableChip();
